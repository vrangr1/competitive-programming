...
void count_pfac(ll num, unordered_map<ll,ll> &fac_counts){
    ll fac = 2ll;
    while(num%fac == 0ll){
        num /= fac;
        fac_counts[fac]++;
    ...
    fac = 3ll;
    for (fac=3ll;fac <= sqrt(num);fac+=2ll){
        while(num%fac == 0ll){
            num /=fac;
            fac_counts[fac]++;
        }
    }
    if (num > 1ll) fac_counts[num]++;
}

void solve(){
    ll n; cin >> n;
    vector<ll> a(n);
    unordered_map<ll,ll> fac_counts;
    repll(i,n){
        cin >> a[i];
        count_pfac(a[i],fac_counts);
    }
    for (auto fac_ct : fac_counts){
        if (fac_ct.second%n) return void(cout << no);
    }
    cout << yes;
}