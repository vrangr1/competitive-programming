...
int find_ops(int t1, int t2, int target){
    if (t1 % target || t2%target) return INT_MAX;
    return t1/target + t2/target - 2;
}

void solve(){
    int a, b, c; cin >> a >> b >> c;
    if (find_ops(a,b,c) <= 3) return void(cout << yes);
    if (find_ops(c,b,a) <= 3) return void(cout << yes);
    if (find_ops(a,c,b) <= 3) return void(cout << yes);
    cout << no;
}