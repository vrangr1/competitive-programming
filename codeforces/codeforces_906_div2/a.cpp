...
void solve(){
    ...
    if (umap.size() > 2) return void(cout << no);
    if (umap.size() == 1) return void(cout << yes);
    int v1 = umap.begin()->second, v2 = umap.rbegin()->second;
    if (abs(v1-v2) <= 1) return void(cout << yes);
