...
void solve(){
    int q; cin >> q;
    set<pair<int,int>> segs, segs2;
    while(q--){
        char op; cin >> op;
        int l, r; cin >> l >> r;
        pair<int,int> seg = make_pair(l,r);
        auto iter = lower_bound(all(segs),seg,[](const pair<int,int> &pr, const pair<int,int> val){
            return pr.first < val.first;
        });
        
        if (iter->first > seg.second){
            cout << no;
        }
        segs.insert(seg);
    }
