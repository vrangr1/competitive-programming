...
void solve(){
    ...
    string num; cin >> num;
    reverse(all(num));
    int ct = count(all(num),'1');
    ll ops = 0ll;
    deque<int> zero_locs;
    rep(i,n){
        if (num[i] == '0') zero_locs.push_back(i);
    }
    rep(i,1,n,1){
        debug(i);
        if (ct > n-i){
        ...
        if (num[i-1] == '1'){
            while(!zero_locs.empty() && (zero_locs.front() <= i-1 || num[zero_locs.front()] == '1'))
                zero_locs.pop_front();
            assert(!zero_locs.empty());
            ops += zero_locs.front()-i+1;
            cout << ops << " \n"[i==n];
            num[zero_locs.front()] = '1';
            zero_locs.pop_front();
            // auto iter = find(num.begin()+i-1, num.end(), '0');
            // assert(iter != num.end());
            // // int ind = iter-num.begin();
            // // ops 
        }
        else{
            cout << "0" << " \n"[i==n];
