...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Sun Oct  6 00:43:34 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
//     auto ins2 = [&](auto &&self, string &s, int ind, trie *node) -> void {
//         rep(i,ind,sz(s)-1,1) {
...
//         trie *node = root;
//         for (char &ch : s) {
//             if (node->cs.find(ch) == node->cs.end()) {
//                 node->cs[ch] = new trie(node);
//                 // if (ch != '?') node->uct--;
//             }
//             node = node->cs[ch];
//             node->ct++;
...
void solve(int test_case) {
    cout << "Case #" << test_case << ": ";
    auto upd = [&](trie *node, bool q = false) -> void {
        if (!q) {
            node->ct = 1;
            for (auto [ch,nd] : node->cs) {
                node->ct += nd->ct;
            }
            return;
        } else {
    ...
    auto insert = [&](auto &&self, string &s, int i, trie *node, bool q = false) -> void {
        if (i == sz(s)) return;
        char ch = s[i];
        if (ch != '?') {
            if (node->cs.find(ch) == node->cs.end())
                node->cs[ch] = new trie(node);
            self(self,s,i+1,node->cs[ch], false);
            upd(node, false);
        } else {
            if (node->cs.find(ch) == node->cs.end()) {
                mint cur = 26;
