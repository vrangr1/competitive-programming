...
void solve(){
    ...
    forn(i, n) cin >> f[i];
    sort(f.begin(), f.end());
    auto iter = lower_bound(f.begin(), f.end(), p/d);
    if (iter == f.end())
        return void(cout << (ll)(accumulate(f.begin(), f.end(), 0)) << endl);
    vector<double> psum(n);
    partial_sum(f.begin(), f.end(), psum.begin());
    if (iter == f.begin()){
        ll div = ((int)n)%(int)d;
        ll sum = ((ll)n)/((ll)d) * (ll)p;
        sum += min((ll)psum[div-1], (ll)p);
        cout << sum << endl;
        return;
    }
    ll count = iter - f.begin();
    ll rem = n - count;
    ll div = rem%(ll)d;
    #if debug
        print_var(count);
        ...
        print_var(div);
    #endif
    if (div == 0) return void(cout << (ll)(psum[count-1] + ((ll)(rem/d))*(ll)p) << endl);
    ll sum = ( ((ll)rem)/((ll)d)*(ll)p) + (ll)psum[count - (d-div) - 1];
    #if debug
        print_var(sum);
    #endif
    sum += min((ll)p, (ll)psum[count+div-1] - (ll)psum[count-d+div-1]);
    cout << sum << endl;
}