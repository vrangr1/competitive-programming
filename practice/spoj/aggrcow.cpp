...
bool check(const vector<int> &x, int n, int c, int dist){
    int last = x[0];
    c--;
...
void solve(){
    ...
    rep(i,n) cin >> x[i];
    sort(all(x));
    int mn = *min_element(all(x)), mx = *max_element(all(x));
    int low = 1, high = (mx-mn)/(c-1) + 2, mid;
    while(low <= high){
        if (low == high) return void(cout << low << endl);
        mid = (low+high)/2;
        if (check(x, n, c, mid)){
            if (low == mid) return void(cout << low << endl);
            low = mid;
        }
        else
            high = mid;
