...
ll compute(ll a, ll b, ll p, ll q, const ll n, const ll m){
    assert(abs(a-p) + abs(b-q) == 1);
    if (!is_valid(p,q,n,m)) return 0;
    if (a==p){
        if (b < q) return n*(m-q);
        else return n*(q+1ll);
    }
...
void solve(){
    ...
    int x, y; cin >> x >> y; --x; --y;
    ll sol = 0ll;
    sol = max(sol, compute(x,y,x+1,y,n,m));
    sol = max(sol, compute(x,y,x,y+1,n,m));
    sol = max(sol, compute(x,y,x-1,y,n,m));
    sol = max(sol, compute(x,y,x,y-1,n,m));
    cout << sol << endl;
}