...
void solve(){
    ...
    if (k < n){
        ...
        else mx = (len/2ll)*(len/2ll);
        if ((n%k) == k-1){
            cout << ind << endl;
            return;
        }
        ll diff = llabs(k-1ll - n%k - 1ll);
        diff /= 2ll;
        if (n%k + 1ll + diff < k){
            len = ((n%k+1ll)%k + k-1ll);
            ll curmx;
            if (len%2ll) curmx = (len/2ll)*(len-(len/2ll));
            else curmx = (len/2ll)*(len/2ll);
            if (curmx > mx)
            return void(cout << n%k + 1ll + diff << endl);
            cout << ind << endl;
            return;
        }
        ll curmx = (k-1ll)*((n-k-1ll)%k);
        if (curmx > mx) return void(cout << k-1ll << endl);
        cout << ind << endl;
        return;
    }
    if (n%2){
