...
public:
	...
	bool exceedsBounds(string num){
		int n = num.size();
		string eval;
		if (num[0] == '-') eval = num.substr(1, n-1);
		else eval = num.substr(0, num.size());
		// 2147483647
		int curNum = 0, maxDigits = 10, maxVal = 147483647;
		if (eval.size() < maxDigits) return false;
		if (eval[0] > '2') return true;
		else if (eval[0] < '2') return false;
		string newStr = eval.substr(1, eval.size() - 1);
		int newNum = stoi(newStr);
		return (newNum > maxVal);
	}

    int reverse(int x) {
        int ans;
        string xstr = to_string(x);
        if (xstr[0] != '-')  std::reverse(xstr.begin(), xstr.end());
        else std::reverse(xstr.begin() + 1, xstr.end());
        if (!exceedsBounds(xstr)) return stoi(xstr);
       	else return 0;
    }
};
