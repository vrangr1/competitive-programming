...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Sun Jun  9 08:33:52 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
public:
    ...
    int rew(vector<ll> &a) {
        sort(all(a));
        vector<ll> mx(a.back()+2ll,-1ll);
        auto ind = [&](ll val) -> ll {
            return (ll)(lower_bound(all(a),val) - a.begin());
        };
        auto get = [&](auto &&self, ll bd) -> ll {
            if (mx[bd] != -1) return mx[bd];
            ll i = ind(bd);
            if (i == 0) return mx[bd] = 0;
            i--;
            assert(a[i] < bd);
            ll temp = self(self,a[i]);
            if (temp+a[i] < bd) mx[bd] = temp+a[i];
            else {
                mx[bd] = a[i];
                mx[bd] = max(mx[bd],a[i]+self(self,bd-a[i]));
            }
            if (bd > 0) mx[bd] = max(mx[bd],self(self,bd-1ll));
            return mx[bd];
        };
        get(get,a.back());
        debug(mx);
        return a.back() + get(get,a.back());
    }
};
