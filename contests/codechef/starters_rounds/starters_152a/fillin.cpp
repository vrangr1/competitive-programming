...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Wed Sep 18 20:53:07 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void solve() {
    ...
    int n; cin >> n;
    int n2 = 2*n;
    vector<int> p(n2), rem(n2+1,1), fixed(n2+1,0);
    int k2 = 0, f = 0;
    rep(i,n2) {
        cin >> p[i];
        if (i%2) {
    ...
    int both = k2-2*f;
    assert(both >= 0);
    assert(k >= fd[0] && k >= fd[1]);
    assert(both/2 <= k-fd[0] && both/2 <= k-fd[1]);
    mint sol = ncr(k-fd[1],fd[0]) * factorial[fd[0]];
    sol *= ncr(k-fd[0],fd[1]) * factorial[fd[1]];
    debug(both,sol());
    // assert(k-fd[0]-fd[1] == both/2);
    debug(factorial[both/2]());
    debug(mint(2).power(both/2)());
    sol *= factorial[both/2] * mint(2).power(both/2) * factorial[both/2];
    cout << sol << endl;
}