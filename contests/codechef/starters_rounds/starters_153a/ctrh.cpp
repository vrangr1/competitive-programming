...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Wed Sep 25 21:10:01 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void solve() {
    ...
    auto dfs = [&](auto &&self, int u, int p) -> mint {
        ...
        for (int &v : g[u]) {
            if (v == p) continue;
            mint cd = self(self, v, u);
            ct++;
            sol*=cd;
        }
        if (ct == 0) sol = 1;
        else if (ct == 1 && p == -1) sol = 2*sol;
        else if (ct == 1) sol = 1*sol;
        else if (p != -1) sol = (mint(2).power(ct)-1)*sol;
        else sol = (mint(2).power(ct)-2)*sol;
        debug(endl,u,p,ct,sol());
        return sol;
    };
    mint sol = dfs(dfs,0,-1);
    sol*=3;
    cout << sol << endl;
}