...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Wed Oct 30 21:17:09 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void solve() {
    int n, k; cin >> n >> k;
    if (k == 1) return void(cout << n/2 << endl);
    mint sol = ncr(n-2,k-1)*(mint(n/2));
    debug(sol);
    rep(i,1,n-2,1) {
        ...
        rep(ind,i-1,1,-2) {
            ...
            if (rem < k-3) break;
            sol += ncr(rem,k-3);
            if (DEBUG) cur += ncr(rem,k-3);
        }
        int rem = n-i-1;
        if (rem >= k-2 && i%2) {
            sol += ncr(rem,k-2);
            if (DEBUG) cur += ncr(rem,k-2);
        }
