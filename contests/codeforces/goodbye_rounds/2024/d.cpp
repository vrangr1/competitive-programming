...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Sat Dec 28 21:02:59 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
private:
    ...
    inline void normalize() {
        this->value = (this->value >= 0) ? this->value % mod() : (this->value % mod() + mod());
    }
    ...
    void add(type lhs, type rhs, type &result) const {
        if (__builtin_add_overflow(lhs, rhs, &result)) {
            ...
            result = lhs + (mod() - rhs); // ensures 0 <= result <= mod
        }
    ...
    inline void subtract(type lhs, type rhs, type &result) const {
        result = (lhs >= rhs ? lhs - rhs : mod() - (rhs - lhs));
    }
...
public:
    ...
    ModularInt& operator++() {
        this->value = add(this->value,1%mod());
        return *this;
    }
    
    ModularInt& operator--() {
        this->value = subtract(this->value,1%mod());
        return *this;
    }
...
void solve() {
    int n, q; cin >> n >> q;
    vector<vector<mint>> vals(2,vector<mint>(n));
    vector<vector<int>> order(2,vector<int>(n)), p(2,vector<int>(n));
    vector<map<mint,pair<int,int>>> mps(2);
    rep(k,2) {
        rep(i,n) cin >> vals[k][i];
        iota(all(order[k]),0);
        stable_sort(all(order[k]), [&](const int &i, const int &j) {
            return vals[k][i] < vals[k][j];
        });
        rep(i,n) {
            if (mps[k].find(vals[k][order[k][i]]) == mps[k].end()) {
                mps[k][vals[k][order[k][i]]] = {i,i};
            } else {
                mps[k][vals[k][order[k][i]]].second = i;
            }
            p[k][order[k][i]] = i;
        }
    }
    ...
    mint sol = 1;
    rep(i,n) {
        sol *= min(vals[0][order[0][i]], vals[1][order[1][i]]);
    }
    cout << sol << " ";
    auto upd = [&](int o, int x) -> void {
        assert(mps[o].find(vals[o][x]) != mps[o].end());
        auto [l,r] = mps[o][vals[o][x]];
        if (order[o][p[o][x]] != x) {
            debug(o,x);
            exit(0);
        }
        assert(order[o][p[o][x]] == x);
        swap(order[o][r], order[o][p[o][x]]);
        swap(p[o][order[o][r]],p[o][order[o][p[o][x]]]);
        x = order[o][r];
        if (l == r) {
            mps[o].erase(vals[o][x]);
        } else {
            mps[o][vals[o][x]].second--;
        }
        sol /= min(vals[0][order[0][r]],vals[1][order[1][r]]);
        vals[o][x]++;
        if (mps[o].find(vals[o][x]) == mps[o].end()) {
            mps[o][vals[o][x]] = {r,r};
        } else {
            assert(mps[o][vals[o][x]].first == r+1);
            mps[o][vals[o][x]].first = r;
        }
        sol *= min(vals[0][order[0][r]],vals[1][order[1][r]]);
    };
    while(q--) {
