...
void generate_test() {
    ll sol = grng(maxn)+2ll;
    ll r = grng(sol), l = grng(r+1ll);
    vector<ll> sel(sol,0ll);
    fill(sel.begin(),sel.begin()+grng(sol)+1ll, 1ll);
    shuffle(all(sel),rng);
    string s;
    rep(i,sol) s.push_back((char)(grng(26)+'a'));
    cout << s.substr(l,r-l+1ll) << endl;
    rep(i,sol) if (sel[i]) cout << s[i];
    cout << endl;
    cout << s << endl;
}