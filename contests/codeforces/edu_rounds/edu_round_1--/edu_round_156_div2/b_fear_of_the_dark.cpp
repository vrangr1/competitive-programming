...
void solve(){
    pair<double,double> p, a, b, z(0.0,0.0);
    cin >> p.first >> p.second;
    cin >> a.first >> a.second >> b.first >> b.second;
    double w = max(get_dist(a,p), get_dist(a,z));
    w = min(w, max(get_dist(b,p),get_dist(b,z)));
    double temp = get_dist(a,b)/(double)2.0;
    double t2 = max(get_dist(z,a), get_dist(p,b));
    t2 = min(t2,max(get_dist(z,b), get_dist(p,a)));
    temp = max(temp, t2);
    w = min(temp,w);
    cout << fixed << setprecision(8) << w << endl;
}