...
const ll maxn = 10, maxq = 5;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test(){
    ll n = grng(maxn)+1ll, q = grng(maxq)+1ll;
    cout << n << space << q << endl;
    rep(i,n) cout << grng(2)+1ll << " \n"[i==n-1];
    rep(i,q){
        ll op = grng(2)+1ll;
        if (op == 1){
            ll s = grng(2*n)+1ll;
            cout << op << space << s << endl;
        }
        else{
            ll j = grng(n)+1ll, v = grng(2)+1ll;
            cout << op << space << j << space << v << endl;
        }
    }
}