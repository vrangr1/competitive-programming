...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Sat Apr  6 20:17:35 IST 2024
****************************************************/
#include <iostream>
#include <vector>
#include <string.h>
...
const ll maxn = 10, maxv = 20;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test(){
    ll n = grng(maxn)+1ll, k = grng(n)+1ll;
    vector<bool> vals(maxv,false);
    fill(vals.begin(),vals.begin()+n,true);
    shuffle(all(vals),rng);
    vector<ll> a;
    rep(i,(ll)maxv) if (vals[i]) a.push_back(i+1ll);
    shuffle(all(a),rng);
    cout << n << " " << k << endl;
    print_vec(a);
}