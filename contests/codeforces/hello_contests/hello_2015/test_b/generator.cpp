...
const int maxn = 1e2, maxq = 1e2, maxa = 1e9, maxb = 1e9, maxf1 = 1e9, maxf2 = 1e9, maxx = 1e9;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test(){
    int n = grng(maxn)+1, q = grng(maxq)+1;
    cout << n << " " << q << endl;
    cout << grng(maxf1)+1 << " " << grng(maxf2)+1 << endl;
    cout << grng(maxa) + 1 << " " << grng(maxb)+1 << endl;
    rep(i,n) cout << grng(maxx) << " \n"[i==n-1];
    while(q--){
        int r = grng(n)+1, l = grng(r)+1;
        cout << l << " " << r << endl;
    }
}