...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Fri May 31 17:08:57 IST 2024
****************************************************/
#include <iostream>
#include <vector>
#include <string.h>
...
void generate_test(){
    ...
    rep(i,1,n-1ll,1) {
        ...
        if (grng(2ll)) {
            a[i] = a[i-1]*2ll + grng(2ll);
        }
        else a[i] = a[i-1]/2ll;
    }
    vector<ll> keep(n,1ll);
    ll len = grng(n+1ll);
    fill(keep.begin(),keep.begin()+len,0ll);
    shuffle(all(keep),rng);
    rep(i,n) {
        if (keep[i]) continue;
        a[i] = -1;
    }
