...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Mon Apr  8 20:33:53 IST 2024
****************************************************/
#include <iostream>
#include <vector>
#include <string.h>
...
const ll maxn = 5, maxv = 5;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test(){
    ll n = grng(maxn)+1ll, m = grng(n)+1ll, k = grng(m)+1ll;
    cout << n << " " << m << " " << k << endl;
    rep(i,n) cout << grng(maxv)+1ll << " \n"[i==n-1];
    rep(i,m) cout << grng(maxv)+1ll << " \n"[i==m-1];
}