...
void check(){
    ...
    [[maybe_unused]] auto read_output = [&](vector<string> &output) -> void {
        ...
        ss >> la;
        if (la == -1) return void(output.push_back("-1\n"));
        ss >> ra >> lb >> rb >> lc >> rc;
        ...
        lc--;
        rc--;
        if (!(la >= 0ll && la <= ra && ra < n)) {
            exit(0);
        }
        assert(la >= 0ll && la <= ra && ra < n);
        assert(lb >= 0ll && lb <= rb && rb < n);
        assert(lc >= 0ll && lc <= rc && rc < n);
        ll sa = 0, sb = 0ll, sc = 0ll;
        rep(i,la,ra,1) sa += a[i];
        rep(i,lb,rb,1) sb += b[i];
        rep(i,lc,rc,1) sc += c[i];
        if (sa < tot || sb < tot || sc < tot) {
            output.push_back(no);
        }
        else
