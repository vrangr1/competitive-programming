...
void generate_test() {
    ...
    rep(i,n) {
        ...
        assert(tot-cur > n-i);
        a[i] = max(1ll,(ll)grng(tot-cur-(n-i)+1ll) + 1ll);
        cur += a[i];
        assert(tot-cur >= n-i-1ll);
    }
    cur = 0ll;
    rep(i,n) {
        assert(tot-cur >= n-i);
        ...
        assert(tot-cur > n-i);
        b[i] = max(1ll,(ll)grng(tot-cur-(n-i)+1ll) + 1ll);
        cur += b[i];
        assert(tot-cur >= n-i-1ll);
    }
    cur = 0ll;
    rep(i,n) {
        ...
        assert(tot-cur > n-i);
        c[i] = max(1ll,(ll)grng(tot-cur-(n-i)+1ll));
        cur += c[i];
        assert(tot-cur >= n-i-1ll);
    }
    print_vec(a);
