...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Thu Oct 24 20:54:54 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void solve() {
    ...
    vector<pair<ll,ll>> fibs;
    fibs.emplace_back((1%k),0);
    vector<ll> inds(k,-1ll);
    inds[(1%k)] = 0;
    if (k == 1ll) fibs.back().second = 1ll;
    ll last = (1ll%k);
    while(true) {
        ll nxt = (last+fibs.back().first);
        ll ct = fibs.back().second;
        if (nxt >= k) {
            nxt%=k;
            ct++;
        }
        last = fibs.back().first;
        if (inds[nxt] == -1) break;
        inds[nxt] = sz(fibs);
        fibs.emplace_back(nxt,ct);
    }
