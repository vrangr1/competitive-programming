...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Sat Jul 20 20:16:27 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void check(){
    ...
    process_input();
    [[maybe_unused]] auto read_output = [&](vector<string> &output, bool brute = false) -> void {
        ...
        vector<ll> a(n);
        rep(i,n) ss >> a[i];
        ll mx = LLONG_MIN, cx = -1ll, sum = 0ll;
        rep(i,n) {
            sum += a[i];
            if (sum > mx) {
                mx = sum;
    ...
    auto process_output = [&](vector<string> &output, bool brute = false) -> void {
        assert(!transfer());
        while(transfer())
            trash_stream();
        read_output(output,brute);
    };
    vector<string> test_output, correct_output;
    process_output(test_output);
    process_output(correct_output, true);
