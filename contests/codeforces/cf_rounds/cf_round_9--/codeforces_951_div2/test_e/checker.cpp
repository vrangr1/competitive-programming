...
void check(){
    ...
    auto take_input = [&]() -> void {
        ss >> n;
        ss >> n >> d;
        debug(n,d);
        // while(!ss.eof()) ss >> s;
        pts.resize(n);
        rep(i,n) ss >> pts[i].first >> pts[i].second;
        debug(pts);
    };
    ...
    auto process_output = [&](vector<string> &output) -> void {
        ...
        assert(k <= n);
        --i;--j;--k;
        debug(i,j,k,dist(i,j),dist(j,k),dist(i,k));
        int ct = (dist(i,j) == d) + (dist(j,k) == d) + (dist(i,k) == d);
        if (ct >= 2) output.push_back("1");
