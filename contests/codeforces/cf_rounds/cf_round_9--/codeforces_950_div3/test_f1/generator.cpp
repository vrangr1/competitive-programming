...
const ll maxn = 5, maxm = 5, maxk = 8;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test(){
    ll n = grng(maxn)+2ll, m = grng(maxm)+2ll, k = grng(min(n*m,maxk));
    if (!k) k += 2ll;
    cout << n << " " << m << " " << k << endl;
    ll nm = n*m;
    assert(k < nm);
    vector<bool> fs(nm,false);
    fill(fs.begin(),fs.begin()+k,true);
    shuffle(all(fs),rng);
    ll t = (n-1ll)*m;
    if (fs[t]) {
        rep(i,nm) {
