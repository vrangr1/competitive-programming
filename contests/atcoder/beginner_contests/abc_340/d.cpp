...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Sat Feb 10 17:54:36 IST 2024
****************************************************/
#include <iostream>
#include <vector>
#include <string.h>
...
void solve(){
    ...
    rep(i,n-1ll){
        ...
        edges[i].push_back({i+1,a});
        edges[i].push_back({x,b});
    }
    auto dijkstra = [&](ll src, ll dest) -> ll {
        ...
        while(!pq.empty()){
            ...
            pq.pop();
            ll w = cur[0], node = cur[1];
            if (node == dest) return w;
            for (auto &edge : edges[node]){
                ll ngb = edge[0];
                ll ew = edge[1];
                if (dp[ngb] < w+ew) continue;
                dp[ngb] = w+ew;
                pq.push({dp[ngb],ngb});
            }
        }
