...
* Author  : Anav Prasad
* Handle  : vrangr
* Created : Sat Sep 21 18:15:22 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
void solve() {
    ...
    rep(i,m) {
        int x, y; cin >> x >> y;
        --x;--y;
        if (chk[x].find(y) != chk[x].end()) continue;
        g[x].push_back(y);
        chk[x].insert(y);
    }
    int lgk = 32-__builtin_clz(k);
    vector<vector<mint>> dp(n,vector<mint>(lgk));
    int cur = 0, val = 1;
    rep(i,n) dp[i][cur] = deg[i];
    cur++;
    val*=2;
    while(val <= k) {
        rep(u,n) {
    ...
    auto get = [&](int u, int kp) -> mint {
        mint sol = 0;
        assert(kp > 0);
        int s = (kp&(-kp));
        sol = dp[u][31-__builtin_clz(s)];
        if (kp == s) return sol;
