...
* Author  : Anav Prasad
* Nick    : vrangr
* Created : Sat Sep  7 18:38:12 IST 2024
****************************************************/
#include <bits/stdc++.h>
...
const ll maxn = 5, maxk = 5, maxa = 5;

// Generate just a singular test. The multitudinous nature of tests are implemented through a bash script
void generate_test() {
    ll n = grng(maxn) + 1ll, k = ll(grng(maxk))*(grng(2)?1ll:-1ll);
    cout << n << " " << k << endl;
    rep(i,n) cout << ll(grng(maxa))*(grng(2)?1ll:-1ll) << " \n"[i==n-1];
}